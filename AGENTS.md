...
- Site-visit analytics already exist and are self-hosted (no third-party service
  needed): the frontend pings `POST /api/analytics/hit` on page load, and
  `GET /api/analytics/summary` (surfaced as a "Site visits" card on the Profile
  page) returns pageviews/sessions per day plus recent Analyze-lookup searches.
  This dashboard is hidden unless the `ANALYTICS_ADMIN_EMAIL` env var is set to
...
- Beyond pageviews, GA also gets custom events for the actual product funnel:
  `sign_up`/`login` (from `finishLogin`, using the backend's `is_new_account`
  flag so Google sign-in is correctly attributed), `analyze_repertoire` (from
  `applyReport`), `begin_checkout`/`purchase` (from `startCheckout`/
  `refreshMeAfterBilling` — `purchase` fires on Stripe Checkout return, before
  the 3-day trial actually charges anything, since there's no server-side
  Measurement Protocol wiring yet), `game_review` (from
  `requestGameAnnotations`), and `practice_start` (once per practice session,
  not per move). A `plan` (`free`/`pro`) user property is set on every
  `setUser()` call via `gaSetUserProperties`. All go through the `gaEvent()`
  helper, which no-ops safely when `gtag` isn't loaded.
